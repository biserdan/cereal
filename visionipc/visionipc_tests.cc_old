...
TEST_CASE("Send single buffer"){
  ...
  *((uint64_t*)buf->addr) = UINT64_MAX; // 8 byte --> first eight bytes
  *(((uint8_t*)buf->addr)+100*100*3-1) = 1; // 1 byte --> last byte

  // read content of buffer file
  printf("content at start:\n" );
  ...
  // read content of buffer file
  // printf("content recv_buf:\n");
  // print_file(recv_buf->fd);

  REQUIRE(recv_buf != nullptr);
  REQUIRE(*(uint64_t*)recv_buf->addr == UINT64_MAX);
  REQUIRE(*(((uint8_t*)recv_buf->addr)+100*100*3-1) == 1);
