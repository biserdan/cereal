...
TEST_CASE("Send single buffer"){
  ...
  *((uint64_t*)buf->addr) = 1234;
  ...
  uint8_t *read_buffer = (uint8_t *)malloc(sizeof(uint8_t)*100*100*3/2);
  cudaMemcpy(read_buffer, buf->buf_cuda, 100*100*3/2, cudaMemcpyDeviceToHost);;
